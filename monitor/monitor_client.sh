...
serv=`mysql --login-path=cksrv -e "select id from servicos where status_id=1 AND servidor_id=$SERVIDOR_ID" --database $SQL_DATABASE |sed 1d`
...
for i in "${arr[@]}"; do 
	...
	if [ $TIPO_SERVICO -eq 2 ]; then
		...
		mysql --login-path=cksrv -e "UPDATE servicos SET resultado=$LOAD, modified='$MODIFIED' where id=$ID;" --database $SQL_DATABASE
	fi
	...
	if [ $TIPO_SERVICO -eq 3 ]; then
		...
		# Da um insert na base com os dados
		mysql --login-path=cksrv -e "UPDATE servicos SET resultado=$USER, modified='$MODIFIED' where id=$ID;" --database $SQL_DATABASE
	fi
	...
	if [ $TIPO_SERVICO -eq 4 ]; then
		PROCESS=`ps -ef |wc -l`
		# Da um insert na base com os dados
		mysql --login-path=cksrv -e "UPDATE servicos SET resultado=$PROCESS, modified='$MODIFIED' where id=$ID;" --database $SQL_DATABASE
	fi
	...
	if [ $TIPO_SERVICO -eq 5 ]; then
		...
		# Da um insert na base com os dados
		mysql --login-path=cksrv -e "UPDATE servicos SET resultado=$PROCESS_Z, modified='$MODIFIED' where id=$ID;" --database $SQL_DATABASE
	fi
	...
	if [ $TIPO_SERVICO -eq 6 ]; then
		...
		Y=$(($X / $MAX_SIZE))
		# Da um insert na base com os dados
		mysql --login-path=cksrv -e "UPDATE servicos SET resultado=$Y, modified='$MODIFIED' where id=$ID;" --database $SQL_DATABASE
	fi
done
